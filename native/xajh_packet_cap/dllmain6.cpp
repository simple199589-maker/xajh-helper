// xajh_packet_cap6.dll - capture RC4 key (SetKey) + Octets (Update entry).
...
// frame types
enum { FT_OCT = 1, FT_KEY = 2, FT_PAIR = 3, FT_SNAPSHOT = 5, FT_DIAG = 6 };
#pragma pack(push, 1)
typedef struct {
  uint32_t self; uint32_t octets; uint32_t begin; uint16_t length;
  uint8_t rc4_i; uint8_t rc4_j; uint8_t phase; uint8_t reserved;
  uint8_t sbox[256]; uint8_t data[1024];
} Snapshot;
typedef struct { uint32_t self; uint32_t octets; uint32_t begin; uint32_t end; uint32_t length; uint8_t raw[32]; } Diag;
#pragma pack(pop)
...
static void __declspec(naked) HookSetKey(void) {
  __asm {
    pushad
    pushfd
    // entry esp=E, push ecx makes the argument [E+4]; after pushad/pushfd it is [esp+0x28]
    mov eax, [esp+0x28]
    push eax
    call cap_record_key
    add esp, 4
...
static void __declspec(naked) HookUpdate(void) {
  __asm {
    pushad
    pushfd
    // record pair: after pushad/pushfd, the original argument is [esp+0x28]
    mov eax, [esp+0x28]
    push eax
    push ecx
...
void cap_record_pair(void* self, void* oct) {
  ...
  __try {
    Diag diag = {};
    diag.self = (uint32_t)s;
    diag.octets = (uint32_t)o;
    diag.begin = *(uint32_t*)((uint8_t*)oct + 4);
    diag.end = *(uint32_t*)((uint8_t*)oct + 8);
    diag.length = diag.end - diag.begin;
    memcpy(diag.raw, oct, sizeof(diag.raw));
    ring_frame(FT_DIAG, &diag, sizeof(diag));
    uint32_t begin = diag.begin;
    uint32_t end = diag.end;
    uint32_t n = diag.length;
    if (begin > 0x10000 && begin < 0x7FFE0000 && end >= begin && n > 0 && n <= 1024) {
      Snapshot snap = {};
      snap.self = (uint32_t)s;
      snap.octets = (uint32_t)o;
      snap.begin = begin;
      snap.length = (uint16_t)n;
      snap.rc4_i = *((uint8_t*)self + 0x108);
      snap.rc4_j = *((uint8_t*)self + 0x109);
      memcpy(snap.sbox, (void*)(s + 8), 256);
      memcpy(snap.data, (void*)(uintptr_t)begin, n);
      ring_frame(FT_SNAPSHOT, &snap, (uint32_t)(sizeof(Snapshot) - 1024 + n));
    }
  } __except (EXCEPTION_EXECUTE_HANDLER) {}
}
...
static DWORD WINAPI WorkerProc(LPVOID) {
  ...
  swprintf_s(g_shm_name, L"Global\\XajhCap6_%u", pid);
  for (int i = 0; i < 200; ++i) {
