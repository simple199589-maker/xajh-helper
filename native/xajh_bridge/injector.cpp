// Minimal LoadLibrary injector for x86 xajh_bridge.dll
// Usage: xajh_inject.exe <pid> <full_path_to_dll>
// @author by ak
...
int wmain(int argc, wchar_t** argv) {
  if (argc < 3) {
    wprintf(L"usage: xajh_inject.exe <pid> <dll_path>\n");
    FlushOut();
    return 2;
  }
  ...
  // Reject non-existent DLL early (clearer than remote LoadLibrary NULL).
  ...
  wprintf(L"inject begin pid=%lu dll=%s\n", (unsigned long)pid, dll);
  FlushOut();
  ...
  // 8s is enough for DllMain + attach worker to create shm; hook is deferred.
  DWORD wait = WaitForSingleObject(thr, 8000);
  if (wait != WAIT_OBJECT_0) {
    // The remote LoadLibrary thread may still be reading `remote`. Never free
    // that page unless the thread is confirmed complete; one small leak on an
    // exceptional inject path is safer than crashing the target process.
    wprintf(L"remote LoadLibrary incomplete wait=%lu; path allocation retained\n",
            (unsigned long)wait);
    FlushOut();
    CloseHandle(thr);
    CloseHandle(h);
  ...
  // Confirm target still alive (game crash during attach).
  DWORD exit_code = 0;
  if (GetExitCodeProcess(h, &exit_code) && exit_code != STILL_ACTIVE) {
    wprintf(L"target process exited during inject code=%lu\n",
  ...
  wprintf(L"injected ok module=0x%lX\n", (unsigned long)code);
  FlushOut();
  return 0;
}
