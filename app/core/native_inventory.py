...
# Fixed-name artifacts that must exist in a native bin directory. The versioned
# bridge (xajh_bridge_<BRIDGE_BUILD_ID>.dll) is resolved lazily below because its
# name embeds the build id.
...
def stamped_bridge_name() -> str:
    """Versioned production bridge filename, e.g. xajh_bridge_2026082603.dll."""
    from app.core.bridge_protocol import BRIDGE_BUILD_ID

    return f"xajh_bridge_{BRIDGE_BUILD_ID}.dll"


def required_names() -> tuple[str, ...]:
    """Every native file that must be present in a bin directory."""
    return NATIVE_BIN_FILES + (stamped_bridge_name(),)